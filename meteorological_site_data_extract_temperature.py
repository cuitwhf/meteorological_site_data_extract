...
def ScZd_T_read(file_name, site_name, time_format):
    ...
    while len(data_frame) > 0:
        res.append(data_frame[:19])
        data_frame = data_frame[19:]
    ...
    for temp_data in res:
        ...
        data_result.append(temp_data.split('(')[0])
...
if __name__ == '__main__':
    ...
    output_name = r'meteorological_data_temperature.csv'  # 输出文件路径
    ...
    for site_i in range(0, len(site_name)):
        ...
        for filewalks in os.walk(file_dir):
            for file in filewalks[2]:
                if 'ScZd_T' in file:  # ****此处可以根据文件前缀选择日期范围
                    file_name = os.path.join(file_dir, file)
                    date_time, data_result = ScZd_T_read(
                        file_name, site_name[site_i], time_format)
